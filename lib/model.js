...
var bufferToJSON = function () { return { $$blob: this.toString('hex') }; }
  , originalBufferToJSON = Buffer.prototype.toJSON
  , util = require('util')
 ...
 * Non-treatable edge cases here: if part of the object is of the form { $$date: number }, { $$blob: string } or { $$deleted: true }
 * Its serialized-then-deserialized version it will transformed into a Date or Buffer object respectively
 * But you really need to want it to trigger such behaviour, even when warned not to use '$' at the beginning of the field names...
 */
function checkKey (k, v) {
  if (k[0] === '$' && !(k === '$$date' && typeof v === 'number') && !(k === '$$blob' && typeof v === 'string') && !(k === '$$deleted' && v === true) && !(k === '$$indexCreated') && !(k === '$$indexRemoved')) {
...
function serialize (obj) {
  ...
  // Keep track of the fact that this is a Buffer object
...
function deserialize (rawData) {
  return JSON.parse(rawData, function (k, v) {
    ...
    if (k === '$$blob') { return new Buffer(v, 'hex'); }
...
function compareThings (a, b) {
  ...
  // Buffers
  if (a instanceof Buffer) { return b instanceof Buffer ? compareNSB(a.toString('hex'), b.toString('hex')) : -1; }
  if (b instanceof Buffer) { return a instanceof Buffer ? compareNSB(a.toString('hex'), b.toString('hex')) : 1; }
...
function areThingsEqual (a, b) {
  ...
  // Buffers
  if (a instanceof Buffer || b instanceof Buffer) { return a instanceof Buffer && b instanceof Buffer && a.toString('hex') === b.toString('hex'); }
...
function areComparable (a, b) {
  if (typeof a !== 'string' && typeof a !== 'number' && !util.isDate(a) && a instanceof Buffer &&
      typeof b !== 'string' && typeof b !== 'number' && !util.isDate(b) && b instanceof Buffer) {
    return false;
  }
