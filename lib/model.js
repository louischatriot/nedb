/** Constructor. Permits the custom ID to be passed in.
*/
...
function modify (obj, updateQuery) {
  ...
  if (keys.indexOf(this.idField) !== -1 && updateQuery[this.idField] !== obj[this.idField]) { throw "You cannot change a document's identifier"; }
  ...
  if (dollarFirstChars.length === 0) {
    ...
    newDoc[this.idField] = obj[this.idField];
  } else {
    // Apply modifiers
  ...
  checkObject(newDoc);
  if (obj[this.idField] !== newDoc[this.idField]) { throw "You can't change a document's _identifier"; }
  return newDoc;
};
