...
var dateToJSON = function () {
  ...
  modifierFunctions = {}, lastStepModifierFunctions = {}, comparisonFunctions = {}, logicalOperators = {}, arrayComparisonFunctions = {};
...
function aggregate(res, group) {
  var nested = {},
    flattened = [],
    graph = traverse(res),
    keys = _.filter(Object.keys(group.key), function (key) {
      return group.key[key];
    }),
    outKeys = [],
    i, j, handle, sub, entry, nodeKeys;
  ...
  for (i = 0; i < res.length; ++i) {
    ...
    for (j = 0; j < keys.length; ++j) {
      sub = new String(graph.get([i].concat(keys[j])));
      if (!handle[sub]) {
        if (j === keys.length - 1) {
  ...
  graph = traverse(nested);
  graph.forEach(function (node) {
    if (this.level === keys.length) {
      if (group.finalize) group.finalize(node);
      entry = {};
      for (i = 0; i < this.path.length; ++i) {
        entry[outKeys[i]] = this.path[i];
      }
