...
function checkKey (k, v) {
  ...
  if (k[0] === '$' && !(k === '$$date' && typeof v === 'number') && !(k === '$$deleted' && v === true) && !(k === '$$expiry') && !(k === '$$indexCreated') && !(k === '$$indexRemoved')) {
...
function isExpired (obj) {
  var expiringKeys, now, i, fieldName, maxAge, val;

  if (obj && obj.$$expiry) {
    expiringKeys = Object.keys(obj.$$expiry);
    if (expiringKeys.length > 0) {
      now = Date.now();
      for (i = 0; i < expiringKeys.length; i += 1) {
        fieldName = expiringKeys[i];
        maxAge = obj.$$expiry[fieldName];
        if (typeof maxAge === 'number' && maxAge >= 0 && maxAge !== Infinity) {
          val = getDotValue(obj, fieldName);
          if (util.isDate(val) && (val.getTime() + (maxAge * 1000)) <= now) {
            return true;
          }
        }
      }
