...
Persistence.prototype.persistCachedDatabase = function (cb) {
  ...
  this.db.getAllData().forEach(function (doc) {
    toPersist += model.serialize(doc, self.db.algorithm, self.db.key) + '\n';
  });
  Object.keys(this.db.indexes).forEach(function (fieldName) {
    if (fieldName != "_id") {   // The special _id index is managed by datastore.js, the others need to be persisted
      toPersist += model.serialize({ $$indexCreated: { fieldName: fieldName, unique: self.db.indexes[fieldName].unique, sparse: self.db.indexes[fieldName].sparse }}, self.db.algorithm, self.db.key) + '\n';
    }
  });
...
Persistence.prototype.treatRawData = function (rawData) {
  ...
  for (i = 0; i < data.length; i += 1) {
    ...
    try {
      doc = model.deserialize(data[i], self.db.algorithm, self.db.key);
      if (doc._id) {
        if (doc.$$deleted === true) {
