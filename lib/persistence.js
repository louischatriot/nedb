...
Persistence.prototype.persistCachedDatabase = function (cb) {
  ...
  //encrypt
  if (self.db.cypher.length > 0 && toPersist.length > 0)
    toPersist = crypto.encrypt(toPersist, self.db.cypher, self.db.key);
      ...
      Persistence.ensureDirectoryExists(path.dirname(self.filename), function (err) {
        self.ensureDatafileIntegrity(function (exists) {
          fs.readFile(self.filename, 'utf8', function (err, rawData) {
            ...
            //decrypt            
            if (rawData.length > 0 && self.db.cypher.length > 0) {
              var data = rawData.split('\n');
              var newData = "";
              for (var i=0; i < data.length; i++) {
                if (data[i].length > 0)
                  newData += crypto.decrypt(data[i], self.db.cypher, self.db.key) + "\n";
              }
              rawData = newData;
            }
