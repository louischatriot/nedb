...
Cursor.pick = function(doc,projection,query){
  ...
  while (i--){//browse projection properties...
    ...
    //we have deeper level
      ...
      tmpDoc = {};
      for (var docKey in doc){
        if (docKey !== key){
          tmpDoc[docKey] = doc[docKey];
        }
      }
      //created temp doc with array property to be replaced with array element in order to pass to match function
      var docValueLen = docValue.length;
      j = -1;
      while(++j < docValueLen){
        tmpDoc[key] = docValue[j];//set it array element property
        if (model.match(tmpDoc,query)){
          copy[key].push(docValue[j]);//add to result set
          break;//leave loop, we only want first element match with positional operator
        }
      }
    }
