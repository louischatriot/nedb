...
function Datastore (options) {
  var filename;
  var defaultIdField = "_id";

  // Retrocompatibility with v0.6 and before
  if (typeof options === 'string') {
    filename = options;
    this.inMemoryOnly = false;   // Default
    this.idField = defaultIdField;
  } else {
    options = options || {};
    filename = options.filename;
    this.inMemoryOnly = options.inMemoryOnly || false;
    this.autoload = options.autoload || false;
    this.idField = options.idField || defaultIdField;
  }

  model.idField = Datastore.idField = this.idField;
  ...
  this.indexes = {};
  this.indexes[this.idField] = new Index({ fieldName: this.idField, unique: true });
...
Datastore.prototype.getAllData = function () {
  return this.indexes[this.idField].getAll();
};
...
Datastore.treatRawData = function (rawData) {
  ...
  for (i = 0; i < data.length; i += 1) {
    ...
    try {
      ...
      if (doc[this.idField]) {
        if (doc.$$deleted === true) {
          delete dataById[doc[this.idField]];
        } else {
          dataById[doc[this.idField]] = doc;
        }
      }
    } catch (e) {
    }
...
Datastore.prototype._insert = function (newDoc, cb) {
  ...
  try {
    newDoc[this.idField] = customUtils.uid(16);
  ...
  try {
    candidates.forEach(function (d) {
      if (model.match(d, query) && (multi || numRemoved === 0)) {
        numRemoved += 1;
        var doc = {$$deleted: true};
        doc[self.idField] = d[self.idField];
