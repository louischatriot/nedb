...
Datastore.prototype._insert = function (newDoc, cb) {
  ...
  function processDoc(doc) {
    if (!doc || typeof doc !== 'object') {
      throw "Invalid type of object to insert";
    }
    doc._id = customUtils.uid(16);
    model.checkObject(doc);
    insertedDocs.push(model.deepCopy(doc));
  }

  try {
    if (util.isArray(newDoc)) {
      newDoc.forEach(processDoc);
    } else {
      processDoc(newDoc);
    }
  } catch (e) {
  ...
  try { self.addToIndexes(insertedDocs); } catch (e) { return callback(e); }

  this.persistence.persistNewState(insertedDocs, function (err) {
