...
Datastore.prototype._update = function (query, updateQuery, options, cb) {
  ...
  , function () {   // Perform the update
    ...
    if(multi && (undefined !== self.skip || undefined !== self.limit)) {
        candidates = self.__aggregate(candidates);
    }
...
Datastore.prototype.__aggregate = function (candidates) {
    ...
    if(undefined !== this.sort) {
        candidates = candidates.sort(function(a, b) {
            for(orderBy in self.sort) {
                orderDirection = self.sort[orderBy];
...
Datastore.prototype._aggregate = function (query, cb) {
    ...
    if(query.hasOwnProperty('$sort') && _.isObject(query['$sort'])) {
        _.forEach(query['$sort'], function(value) {
            if(value !== -1 && value !== 1) {
    ...
    if(query.hasOwnProperty('$sort')) {
        if(_.isObject(query['$sort'])) {
            ...
            this.sort = {};
            this.sort[query['$sort']] = 1; //Default sorting
        }
    }
