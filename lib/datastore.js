...
Datastore.prototype.aggregate = function (pipeline, options, cb){
  var datastore = this;
  async.reduce(pipeline,null,function(dataset,operator,callback){
    if (!!operator.$match){

    }
    else{//unknown operator
      var err = new Error('Unknown aggregation operator '+Object.keys(operator)[0]+' used.');
      callback(err);//return with error to callback
    }
  },cb);
};
