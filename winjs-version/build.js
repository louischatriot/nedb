...
child_process.exec('npm install', { cwd: __dirname }, function (err, stdout, stderr) {
  ...
  , function (cb) {
    console.log("Copying winjs specific files to replace their server-specific counterparts");

    async.eachSeries(fs.readdirSync(path.join(__dirname, 'winjs-specific')), function (item, _cb) {
      fs.copy(path.join(__dirname, 'winjs-specific', item), path.join(__dirname, 'src', item), _cb);
    }, cb);
  }
  , function (cb) {
