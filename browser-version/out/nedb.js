...
// export static access to Persistence, to allow access to Persistence.setStorage()
...
/**
 * Replace the storage instance with a custom instance.
 * 
 * In your application:
 *   require("nedb").Persistence.setStorage(...);   
 * 
 * The instance must provide all the functions that are implemented in the
 * default storage.js
 * 
 * Swapping the storage engine should be done before creating any NeDB instance
 * to avoid nasty side effects.    
 * 
 * Currently this can only be set globally (note this is a static method) 
 * because the static Persistence.ensureDirectoryExists() call is found at
 * various places in the NeDB code. 
 */
Persistence.setStorage = function(newStorageInstance) {
  storage = newStorageInstance;  
}
