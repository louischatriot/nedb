  ...
  describe('Modifying documents', function() {
    ...
    describe('$inc modifier', function() {
      ...
      it('Works recursively', function() {
        ...
        _.isEqual(modified, { some: 'thing', nay: { nope: 38 }, blip: { blop: 123 } }).should.equal(
          true
        );
      });
    }); // End of '$inc modifier'
    ...
    describe('$push modifier', function() {
      ...
      it('Can use the $slice modifier to limit the number of array elements', function() {
        ...
        modified = model.modify(obj, {
          $push: { arr: { $each: ['world', 'earth', 'everything'], $slice: 1 } }
        });
        ...
        modified = model.modify(obj, {
          $push: { arr: { $each: ['world', 'earth', 'everything'], $slice: 0 } }
        });
        assert.deepEqual(modified, { arr: [] });

        modified = model.modify(obj, {
          $push: { arr: { $each: ['world', 'earth', 'everything'], $slice: 2 } }
        });
        assert.deepEqual(modified, { arr: ['hello', 'world'] });
        ...
        modified = model.modify(obj, {
          $push: { arr: { $each: ['world', 'earth', 'everything'], $slice: -20 } }
        });
        assert.deepEqual(modified, { arr: ['hello', 'world', 'earth', 'everything'] });
  ...
  describe('Querying', function() {
    describe('Comparing things', function() {
      ...
      it('Can test objects equality', function() {
        ...
        model
          .areThingsEqual({ hello: 'world' }, { hello: 'world', temperature: 42 })
          .should.equal(false);
        model
          .areThingsEqual(
    ...
    describe('Getting a fields value in dot notation', function() {
      ...
      it('Return undefined if the field cannot be found in the object', function() {
        ...
        assert.isUndefined(
          model.getDotValue({ hello: 'world', type: { planet: true } }, 'type.plane')
        );
      });
    ...
    describe('Field equality', function() {
      ...
      it('Can match for field equality inside an array with the dot notation', function() {
        model
          .match({ a: true, b: ['node', 'embedded', 'database'] }, { 'b.1': 'node' })
          .should.equal(false);
        model
          .match({ a: true, b: ['node', 'embedded', 'database'] }, { 'b.1': 'embedded' })
          .should.equal(true);
        model
    ...
    describe('Regular expression matching', function() {
      ...
      it('Can use dot-notation', function() {
        ...
        model
          .match({ test: { nested: 'true' } }, { 'test.nested': { $regex: /true/ } })
          .should.equal(true);
        model
    ...
    describe('Other comparison operators: $lte, $gt, $gte, $ne, $eq, $in, $exists', function() {
      it('$lte', function() {
      ...
      it('$eq', function() {
        ...
        model.match({ a: 5 }, { a: { $eq: 5 } }).should.equal(true);
        model.match({ a: 5 }, { b: { $eq: 5 } }).should.equal(false);
        model.match({ a: false }, { a: { $eq: false } }).should.equal(true);
      });
    ...
    describe('Comparing on arrays', function() {
      ...
      it('$elemMatch operator works with empty arrays', function() {
        model
          .match({ childrens: [] }, { childrens: { $elemMatch: { name: 'Mitsos' } } })
          .should.equal(false);
    ...
    describe('Logical operators $or, $and, $not', function() {
      it('Any of the subqueries should match for an $or to match', function() {
        model
          .match({ hello: 'world' }, { $or: [{ hello: 'pluton' }, { hello: 'world' }] })
          .should.equal(true);
        model
          .match({ hello: 'pluton' }, { $or: [{ hello: 'pluton' }, { hello: 'world' }] })
          .should.equal(true);
        model
          .match({ hello: 'nope' }, { $or: [{ hello: 'pluton' }, { hello: 'world' }] })
          .should.equal(false);
        model
    ...
    describe('Array fields', function() {
      it('Field equality', function() {
        ...
        model
          .match({ tags: ['node', 'js', 'db'], nedb: true }, { tags: 'js', nedb: true })
          .should.equal(true);
      ...
      it('Works with arrays that are in subdocuments', function() {
        model
          .match({ children: { ages: [3, 7, 12] } }, { 'children.ages': { $lt: 2 } })
          .should.equal(false);
        model
          .match({ children: { ages: [3, 7, 12] } }, { 'children.ages': { $lt: 3 } })
          .should.equal(false);
        model
          .match({ children: { ages: [3, 7, 12] } }, { 'children.ages': { $lt: 4 } })
          .should.equal(true);
        model
          .match({ children: { ages: [3, 7, 12] } }, { 'children.ages': { $lt: 8 } })
          .should.equal(true);
        model
          .match({ children: { ages: [3, 7, 12] } }, { 'children.ages': { $lt: 13 } })
          .should.equal(true);
      });
